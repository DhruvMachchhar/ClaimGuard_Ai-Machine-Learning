...
app.add_middleware(
    CORSMiddleware,
    allow_origins=allowed_origins,
    # Requests from the deployed frontend go through the same-origin "/api"
    # rewrite on Vercel, so CORS isn't hit there. This regex is just a safety
    # net for any Vercel preview/production domain calling the API directly.
...
@api_router.get("/health", response_model=HealthResponse, summary="API Health Check")
def health_check():
    """
    Returns the API service health and verifies that the Random Forest model is loaded in memory.
    """
...
@api_router.get("/model-info", response_model=ModelInfoResponse, summary="Model Metadata & Hyperparameters")
def get_model_info():
    """
    Returns verified configuration, hyperparameters, and feature specifications of the trained model.
    """
...
@api_router.get("/analytics/model", response_model=ModelAnalyticsResponse, summary="Model Evaluation Performance Metrics")
def get_model_analytics():
    """
    Returns verified accuracy, precision, recall, F1 score, and confusion matrix from Model-Creation.ipynb.
    """
...
@api_router.post("/predict", response_model=PredictionResponse, summary="Predict Insurance Claim Fraud Risk")
def predict_claim(claim_input: ClaimPredictionInput):
    """
    Accepts full insurance claim parameters, executes preprocessing, feature scaling,
    and runs the trained Random Forest Classifier to predict fraud risk and anomaly probability.
